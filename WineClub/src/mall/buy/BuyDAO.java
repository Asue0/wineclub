...
public class BuyDAO {
	...
	// 구매목록 추가 - 트랜잭션 처리
	// 3가지의 작업 - 구매 테이블에 구매상품 등록, 카트 테이블 에서 구매 상품을 삭제, 상품 테이블 상품 재고 수량을 수정
	public void insertBuyList(List<BuyDTO> buyList, List<CartDTO> cartList)
			throws SQLException{
		String sql1 = "insert into buy(buy_id, buyer,"
				+ " product_id, product_name, buy_price,"
				+ " buy_count, product_image, account,"
				+ " delivery_name, delivery_tel, delivery_address)"
				+ " values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
		String sql2 = "delete from cart where cart_id = ?";
		String sql3 = "select product_count from product where product_id = ?";
		String sql4 = "update product set product_count = ? "
				+ "where product_id = ?";
		try {
			...
			// 3번 작업 - 상품 테이블 해당 상품 재고 수량을 파악 -> 상품 테이블에서 상품의 재고량을 수정(구매 상품의 개수를 빼서 설정)
			for(BuyDTO buy : buyList) { 				// 상품 테이블에서 구매 상품의 재고량을 파악
				int p_count = 0;
	...
	public List<BuyDTO> getBuyListSearch
	(String period1, String period2, String buyer) {
